...
describe("поправка 2 — вырожденные паузы не зависают", () => {
  ...
  it("реплика без единой паузы (слова встык) режется по стыку там, где стык есть, и по потолку там, где его нет", () => {
    ...
    //
    // Ожидание изменено вместе с фиксом ролика 34 (28.08.2026). Раньше тест
    // фиксировал [0,10], [10,20], [20,25] — ТРИ реза, и все три посреди слова,
    // потому что стык вообще не считался кандидатом. Теперь стык (12.00с)
    // используется на второй итерации: он единственная точка этого входа,
    // которая не разрезает слово. Первый рез (10.00с) всё равно приходится на
    // живое слово — первое слово тут длиной 12с, длиннее потолка модели, и
    // законной точки внутри него не существует физически; последний (22.00с)
    // — по той же причине внутри второго слова.
    //
    // Появившаяся часть [10, 12] длиной 2с — не побочный эффект стыка, а ровно
    // то же, что функция делает с НАСТОЯЩЕЙ паузой на этом месте: на входе
    // [0,12], [12.5,25] ветка 1 даёт часть [10, 12.25] той же длины. Стык
    // ведёт себя как пауза нулевой ширины, а не как отдельное правило.
    const result = splitLongPresenterLine({
    ...
    expect(result.parts).toEqual([
      ...
      { startSec: 12, endSec: 22 },
      { startSec: 22, endSec: 25 },
    ])
    expect(result.interludes).toEqual([])
    expect(result.warning).toMatch(/по стыку слов/)
  })
})
