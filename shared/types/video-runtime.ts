...
/**
 * Частота кадров собранного ролика.
 *
 * К этой сетке нормализация приводит КАЖДЫЙ клип (см. normalizeClip в
 * server/utils/render.ts), поэтому по ней же обязаны считаться границы кусков
 * звука на маршруте «монтаж от звука» (server/utils/voiceover/segment-cut.ts).
 * Пока число жило литералом в рендере и отдельным дефолтом в lip-sync, они
 * совпадали по совпадению: правка одного молча уводила бы звук от картинки на
 * доли кадра в каждой сцене.
 */
