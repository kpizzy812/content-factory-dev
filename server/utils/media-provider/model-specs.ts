 ...
 *
 * ЦЕНА — $3 за успешный прогон. Тариф ПЕРЕЧИТАН СО СТРАНИЦЫ МОДЕЛИ 27.08.2026
 * (`https://replicate.com/minimax/voice-cloning`, `billingConfig` в HTML) и
 * совпал с прежним значением:
 *
 *   metric "generic_output_count", title "per output", price "$3",
 *   description "or around 33 outputs for $100"
 *
 * Поэтому единица `flat` и `billingConfirmed: true` законны: биллинг — за ВЫХОД,
 * а не за секунду железа и не за длину образца. Соседние поля той же страницы
 * (`"price": "$0.0001 per second"`, `"p50price"`) — генерические поля железа
 * (`hardware: CPU`), тарифом модели они не являются: при наличии per-unit тарифа
 * в `billingConfig` счёт идёт по нему, и чтение цены по ним занизило бы её на
 * четыре порядка. Снимок и рабочая команда — `docs/operations/replicate.md`
 * §«Тариф клонирования».
 *
 * До 27.08 цифра держалась только на оплаченном прогоне 15.08.2026 и на
 * `scripts/clone-voice.ts`, то есть на собственном прошлом выводе: страница в
 * той сессии отдавала 403. 403 оказался перемежающимся — при недоступности
 * страницы тариф надо перепроверять позже, а не считать подтверждённым по
 * своему же коду. `integrated: true` в отличие от
