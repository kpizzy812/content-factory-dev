...
export interface LipSyncStepResult {
   ...
   *
   * ПРЕДУПРЕЖДЕНИЕ (ре-ревью 1, Important 2): это `string[]`, а не
   * `LipSyncedClipPath[]`, НАМЕРЕННО — массив в общем случае смешивает пути
   * сцен, реально прошедших lip-sync, с путями сцен без реплики (клип
   * `clip_generation` как есть) и с пустыми строками ролика ведущей до записи
   * результата. Единого признака «этот элемент синхронизирован» здесь нет, и
   * протягивать бренд через ВЕСЬ массив означало бы либо соврать типом на
   * несинхронизированных элементах, либо развернуть смешанный
   * `(string | LipSyncedClipPath)[]`, который потребитель всё равно не сможет
   * сузить без ручной проверки, то есть без пользы.
   *
   * Тот же результат содержит `scenes` (`LipSyncSceneRecord[]`) — там у
   * КАЖДОЙ обработанной сцены `outputPath` типизирован именно
   * `LipSyncedClipPath | null` (см. `presenter/lip-sync-progress.ts`), и для
   * непустого `outputPath` бренд НЕ стёрт. Потребителю шага 8 (сборка PiP)
   * нужен путь конкретной синхронизированной сцены — брать его из
   * `scenes[i].outputPath`, а не из этого массива, и тогда `as LipSyncedClipPath`
