 ...
 *
 * `awaiting_operator` (пошаговый режим, §9) сюда НЕ входит намеренно и добавлен
 * быть не может. Ролик в этом статусе не завис: шаг доведён до конца, блокировка
 * отпущена, процесс завершился — за роликом никого нет ПО ЗАМЫСЛУ, он ждёт
 * человека. Watchdog фильтрует кандидатов прямо в SQL по этому списку
 * (`server/plugins/video-recovery.ts`), поэтому вне списка он такой ролик не
 * увидит вовсе; попади он в список — поднял бы прогон, которого оператор не
 * просил, и оплатил бы следующий шаг сам. Автопродолжения по таймауту у режима
 * нет специально: оно обесценило бы сам режим.
 */
...
/**
 * Статусы, из которых ролик можно отменить вручную.
 *
 * Отдельный список, а не производный от RESUMABLE_VIDEO_STATUSES: возобновляемость
 * и отменяемость решают разные вопросы. Watchdog спрашивает «поднимать ли самому
 * за деньги», отмена — «есть ли что останавливать». Ответы расходятся ровно на
 * `awaiting_operator`.
 *
 * ИНВАРИАНТ, который держит этот список честным:
 * RESUMABLE_VIDEO_STATUSES ⊆ CANCELABLE_VIDEO_STATUSES. Если watchdog вправе
 * ПОДНЯТЬ ролик за наши деньги, оператор обязан иметь возможность его
 * остановить; право поднять без права остановить — асимметрия, а не экономия.
 * Обратное включение не требуется и неверно (см. `awaiting_operator`).
 * Инвариант закреплён тестом `tests/unit/fixes/video-cancel-statuses.spec.ts`,
 * там же таблица по всему enum схемы: каждый статус либо терминальный, либо
 * отменяемый, третьего не бывает.
 *
 * `awaiting_operator` сюда ВХОДИТ, хотя в возобновляемые не входит. Из этого
 * статуса нет ни автоматического, ни временнóго выхода: автопродолжения у режима
 * нет намеренно (§9), watchdog такой ролик не трогает намеренно. Единственный
 * выход ручной — и запрет отмены запирал бы ролик, которого оператор решил не
 * доводить, навсегда: ни принять, ни отменить.
 *
 * `generating_voiceover` и `configuring` добавлены 28.08.2026 — до этого их тут
 * не было НИКОГДА (инлайновый список в `server/api/videos/[id]/cancel.post.ts`),
 * и это была дыра, а не решение:
 *
 *  - `generating_voiceover` — самый длинный шаг маршрута «монтаж от звука»: под
 *    ним идут посценный TTS (вызов НА КАЖДУЮ сцену), синтез единого трека и
 *    ТРАНСКРИПЦИЯ (`video-pipeline-steps.ts` ставит тот же статус с
 *    `currentStep: "transcription"`). Отмена здесь освобождает по-настоящему:
 *    `cancelVideoPipeline` гасит живые `MediaPrediction` ролика через
 *    `provider.cancel` (TTS на `async_prediction`) и fal-задачи шага по
 *    `falRequestId` (TTS на `sync_queue`). Не освобождается только уже ушедший
 *    СИНХРОННЫЙ вызов — Fish (`sync_bytes`) и транскрипция (`sync_json`), у
 *    которых внешнего id до ответа не существует; это цена ОДНОГО вызова против
 *    оплаты всех оставшихся сцен без кнопки.
 *  - `configuring` — статус, который в этом репозитории никто не ПИШЕТ (остался
 *    от миграции 20260407082918). Отменять там нечего ровно потому, что туда
 *    никто не попадает; но он есть в возобновляемых, то есть легаси-строку в
 *    боевой БД watchdog поднимет, а оператор без этой строки остановить бы не
 *    смог. Инвариант выше важнее мёртвой ветки.
 *
 * Терминальные (completed/failed/timeout/canceled/file_missing) отсутствуют:
 * останавливать там нечего, прогона за ними нет.
 *
 * Список по-прежнему выписан ЯВНО, а не собран из RESUMABLE_VIDEO_STATUSES
 * спредом: спред увёл бы `awaiting_operator` из отменяемых при первой же правке
 * возобновляемых, а он там нужен именно потому, что в них не входит.
 */
export const CANCELABLE_VIDEO_STATUSES: readonly string[] = [
  "pending",
  "configuring",
  "generating_prompts",
  "generating_images",
