 ...
 *
 * Список выписан явно, а не собран из RESUMABLE_VIDEO_STATUSES спредом: в
 * возобновляемых есть `configuring` и `generating_voiceover`, которых в
 * отменяемых не было НИКОГДА (`server/api/videos/[id]/cancel.post.ts` до этой
 * правки). Спред тихо расширил бы отмену на два статуса заодно с переносом —
 * правка про пошаговый режим не должна менять поведение обычной отмены. Дырой
 * это выглядит (длинный и дорогой `generating_voiceover` отменить нечем), но
 * закрывать её надо отдельной задачей и отдельным тестом.
 */
