 ...
 *
 * По сути `cutSegment` (ingest) с параметрами в виде объекта, а не позиционно:
 * `buildPresenterCutArgs` длительность не клампит вовсе (ограничивает только
 * разрешение и fps под lip-sync модель), так что клампить её тут неоткуда и
 * незачем. Отдельная функция — чтобы у вызывающего (резервирование окна речи,
 * `presenter-recording-selector.ts`) был свой именованный контракт, а не
 * потому что здесь другая нарезка.
 */
