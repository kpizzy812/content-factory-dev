...
/**
 * Позиционное тождество двух списков сцен: длины совпадают, и `order` каждой
 * сцены совпадает с `order` сцены на ТОЙ ЖЕ позиции второго списка.
 *
 * Единая проверка на два потребителя (`video-pipeline-steps.ts` — якоря подгона
 * длины клипов под трек, `lip-sync-runner.ts` — выбор куска трека для сцены):
 * оба на маршруте «монтаж от звука» сталкиваются с одним и тем же дублем `order`
 * у ролика, снятого целиком ведущей (клипов и порядка их нарезки не существует,
 * позиция сцены в плане — единственный законный адрес). Правило одно, чтобы
 * решение «когда позиции доверять» не разъезжалось по двум переписанным копиям.
 *
 * Тождество ломается НЕ только дублем `order`, но и тремя другими путями,
 * задокументированными потребителями: сцена из одного маркера паузы выпадает во
 * `voiceover/track-builder.ts` (`if (!cleaned) continue`), сцена с пустыми после
 * нормализации токенами — в `transcription/align.ts`, сироты-нарраторы удлиняют
 * список в `voiceover/script-merge.ts`, а сортировка по `order` там же ломает
 * тождество при инверсии `order` в плане. На неподтверждённом тождестве
 * потребитель обязан остаться на прежнем, order-ключуемом сопоставлении: молчаливая
 * позиционная догадка здесь означает чужой кусок трека или чужую позицию в склейке.
 */
export function alignedScenesMatchPlanPositions(
  alignedScenes: readonly SceneOrderLike[],
  planScenes: readonly SceneOrderLike[],
): boolean {
  return alignedScenes.length === planScenes.length
    && alignedScenes.every((scene, sceneIndex) => scene.order === planScenes[sceneIndex]!.order)
}
