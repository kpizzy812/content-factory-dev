...
/**
 * Допуск «рвёт ли граница слово» — ПОЛНЫЙ кадр, а не половина плюс
 * небольшой запас (Minor Н-10 ре-ревью раунда 2, ужесточено по Important
 * НН-6 ре-ревью раунда 3). Раньше запас был фиксированными 3 мс сверх
 * половины кадра — недостаточно: `resolveBoundary` в `repair.ts` умеет
 * ПЕРЕПРОВЕРИТЬ, не завело ли округление к кадру (`snapSecToFrame`, сдвиг до
 * половины кадра в любую сторону) безопасную точку внутрь слова, но фолбэк
 * при провале перепроверки возвращал ТУ ЖЕ небезопасную точку — то есть сама
 * перепроверка была диагностикой без последствий. Чтобы округление НИКОГДА
 * не могло перекинуть точку через границу допуска, допуск обязан быть не
 * меньше СУММЫ двух половин кадра (запас с обеих сторон точки, а не с одной).
 * Замер ре-ревью: на допуске `halfFrameSec + halfFrameSec` число случаев,
 * когда ремонт сам создаёт `word_split` на здоровом входе, падает с 32 до 4
 * (из 27 813 сценариев).
 */
function wordEdgeToleranceSec(fps: number): number {
  return 2 * halfFrameSec(fps)
}
