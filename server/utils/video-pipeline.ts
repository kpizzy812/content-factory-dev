...
export async function runVideoPipeline(
  ...
  try {
    ...
    // ── Ворота маршрута «монтаж от звука»: ни одной сцены ведущего ────────────
    //
    // У сцен ведущего своего клипа нет НИ ОДНОГО: text-to-video по ним не
    // запускался (`presenterSceneIndexes` выше), кадр каждой делает только
    // lip-sync. Ноль синхронизированных сцен на этом маршруте означает ролик, у
    // которого речь звучит непрерывным треком, а показывать под неё нечего:
    // ячейки сцен ведущего остались пустыми, в склейку уходят одни перебивки, и
    // родные дорожки клипов там заглушены в ноль (`clipVolumeWithVoiceoverFor`,
    // spec §6.4). Подгон длины от lip-sync не зависит, ворота сборки такой ролик
    // пропускают — он заливался в хранилище со статусом «готов».
    //
    // Тот же класс брака и та же реакция, что у ruling «транскрипция
    // обязательна» (spec §4.1) и у §10 «шаг падает честно, ролик не помечается
    // готовым»; вход другой — не «нет границ слов», а «границы есть, а вырезать
    // кусок или синхронизировать губы не удалось ни разу» (`track_segment_failed`,
    // отказ провайдера на каждой сцене). Путь не теоретический: первая
    // реализация нарезки в этой ветке роняла ВСЕ вырезки на настоящем ffmpeg.
    //
    // Три границы гейта:
    //  - `audioFirstTrackCompleted` — старого маршрута гейт не касается вовсе:
    //    там сцене без lip-sync остаётся её собственный сгенерированный клип и
    //    посценная озвучка, ролик законен;
    //  - `presenterSceneIndexes.size > 0` — ролик без сцен ведущего (перебивки
    //    под закадровый голос, выключенный lip-sync, отсутствующий персонаж)
    //    синхронизировать нечего и незачем, шаг там штатно skipped/disabled;
    //  - `=== 0` строго — ЧАСТИЧНАЯ синхронизация это штатная деградация, а не
    //    повод падать: несошедшиеся сцены уже отработаны «последним рубежом»
    //    шага (`lip-sync-runner.ts`), и ролик с частью живых кадров лучше
    //    отказа. Снапшот старого формата без `syncedSceneCount` даёт undefined
    //    и гейт не срабатывает — на неизвестности не роняем.
    //
    // Ролику, целиком снятому ведущей, гейт ничего не ломает: пока хоть одна
    // сцена снята, счётчик больше нуля, а при нуле у такого ролика пусты ВСЕ
    // ячейки — склейки не остаётся, и сборка всё равно отказала бы («карта
    // позиций пуста» в `planAlignedClipTargets`). Гейт лишь делает этот отказ
    // раньше и дешевле: музыка и предпроход детектора ключевых слов ещё не
    // оплачены.
    if (audioFirstTrackCompleted
      && presenterSceneIndexes.size > 0
      && lipSyncResult.syncedSceneCount === 0) {
      const message = `Ни одна сцена ведущего не синхронизирована (${presenterSceneIndexes.size} сцен с репликой в кадре): `
        + "на маршруте «монтаж от звука» своего клипа у этих сцен нет — кадр каждой делает lip-sync. "
        + "Под непрерывную речь трека показывать нечего, прогон остановлен, готовым ролик не помечается. "
        + "Причину по каждой сцене смотрите в логе шага lip_sync_generation"
      const lipSyncStep = await ensureStep(videoId, "lip_sync_generation", STEP_ORDER.indexOf("lip_sync_generation"))
      // outputSnapshot НЕ трогаем: в нём лежат записи сцен с причинами отказа —
      // и диагностика оператора, и то, из чего следующий прогон соберёт
      // переиспользование. Меняем только статус и текст ошибки; пофайловый
      // разбор «почему не вышло» шаг уже написал в свой лог сам.
      await updateStep(lipSyncStep.id, {
        status: "failed",
        finishedAt: new Date(),
        errorMessage: message.slice(0, 1000),
      })
      await logAgent('video-pipeline', 'error', `Video ${videoId}: ${message}`, { videoId }).catch(() => {})
