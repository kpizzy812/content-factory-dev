...
/**
 * Референс персонажа не принадлежит ролику, а ключ идемпотентности
 * асинхронного контура строится от videoId (`buildMediaIdentity`). Поэтому
 * здесь допускаются только модели с синхронным исполнением — Replicate под
 * референсы включается отдельным этапом вместе со своей схемой идентичности
 * (см. spec 2026-08-07-replicate-media-contour, шаг 6). Иначе выбор
 * Replicate-модели в UI приводил бы к 500 «требует videoId».
 */
