...
/**
 * AI-автозаполнение полей сцены: промпт → предложения по полям → выбор.
 * Зеркалит CharacterAiAutofill, отличается только схемой (scene_entity).
 *
    ...
    <div v-if="expanded" class="flex flex-col gap-2" :class="compact ? 'px-2.5 pb-2.5' : 'px-3 pb-3'">
      <div class="flex items-end gap-1.5">
        <UiTextarea
          v-model="prompt"
          :rows="textareaRows"
          placeholder="Опишите сцену: что происходит, какая атмосфера. Например: утренняя пробежка в парке, прохладно, золотой час"
          :disabled="loading"
          @keydown="onKeydown"
        />
