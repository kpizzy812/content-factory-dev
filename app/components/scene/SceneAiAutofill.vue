...
/**
 * SceneAiAutofill — AI автозаполнение полей сцены.
 * Используется в SceneCreateModal.vue и SceneComposer.vue.
 *
 * Зеркалит CharacterAiAutofill, но с scene_entity schema (name/description/tags).
 */
...
const props = defineProps<{
  ...
  appId?: number
  /** ID сцены или 'new' (для cacheKey) */
  entityId?: string | 'new'
  /** Компактный режим (модалка): меньшие padding + textarea 2 строки */
  compact?: boolean
}>()
...
const schema = computed(() => nodeFieldSchemas['scene_entity'] ?? {})
      ...
      <div v-if="expanded" :class="['space-y-2', containerPaddingX, containerPaddingY]">
        <div class="flex gap-1.5">
          <textarea
            ...
            :rows="textareaRows"
            placeholder="Опишите сцену: что происходит, какая атмосфера. Пример: «Утренняя пробежка в парке, прохладная погода, золотой час»"
            :disabled="loading"
            @keydown="onKeydown"
          />
